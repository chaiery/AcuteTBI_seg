function  [WSM,winStru, rev] = getWSMFromMassCent_ByCenter_innerB_kmean(center, innerbrain_mask, seg_kmean )
...
% ybottom = uint16(max(ytop + (bbox(4)/2) - 20, cen(2))); ybottom = uint16(max(bbox(2) + (bbox(4)) -60, cen(2)));
...
left_star = max((uint16(cen(1)) - uint16(s_win_width/2)), 1);

for j = ytop:(ybottom-s_win_height)
    for i = left_star:(xright_direction-s_win_width)
...
for j = ytop:(ybottom-s_win_height)
    for i = left_star:-1:xleft_direction
