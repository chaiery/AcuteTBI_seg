function rev = get_li8_ideal_onepersonjuly10_NegarVersion(stru, adjust)
...
% (1) approximate ideal midline detection
num_approximateCenter = 0;
sum_approximateCenter = [0,0];
for i_pos=1:num_stru
    ...
    if stru(i_pos).rev ~= 0
     ...
     num_approximateCenter = num_approximateCenter + 1;
     sum_approximateCenter = sum_approximateCenter + stru(i_pos).center;
end
center_approximate = floor(sum_approximateCenter / num_approximateCenter);
...
stru_good=[];
for i_pos=1:num_stru
    % N: change the function to find the angle anf input from img_Mattress
    % to img_MattressFractured
...
for i_pos = 1:i_pos_good
    stru_good(i_pos).centered_img_Mattress=centerimg_size(stru_good(i_pos).imStru.img_Mattress, center_approximate, new_width, new_height);
    stru_good(i_pos).centered_bwSkullBone =centerimg_size(stru_good(i_pos).bwSkullBone, center_approximate, new_width, new_height);
end

for i_pos=1:i_pos_good
    stru_good(i_pos).rotatedImage_2D=imrotate(stru_good(i_pos).centered_img_Mattress,rotate_angle,'crop');
    stru_good(i_pos).rotatedImage = zeros([size(stru_good(i_pos).rotatedImage_2D),3]);
    for i=1:3
        stru_good(i_pos).rotatedImage(:,:,i) = stru_good(i_pos).rotatedImage_2D;
    end
    stru_good(i_pos).rotatedBWSkull=imrotate(stru_good(i_pos).centered_bwSkullBone,rotate_angle,'crop');
...
sum_realCenter=[0,0];
for i_pos=1:i_pos_good 
    k=regionprops(stru_good(i_pos).rotatedBWSkull,'Centroid');
    stru_good(i_pos).center=k.Centroid;
    sum_realCenter = sum_realCenter + stru_good(i_pos).center;
end
center=floor(sum_realCenter / i_pos_good);



fprintf('    (1) result of ideal midline detection:\n        rotate_angle = %d; center = [ %d, %d ] \n',rotate_angle, center(1), center(2) );
...
%     m_y = adjust_Ang_center(3);
    ...
    rota_bwSkullBone = stru_good(i_pos).rotatedBWSkull;
    rota_original_img = stru_good(i_pos).rotatedImage;
    
    rota_original_img=centerimg(rota_original_img,center,'crop');
    rota_bwSkullBone=centerimg(rota_bwSkullBone,center,'crop');
