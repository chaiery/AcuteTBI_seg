...
for i = 1:length(ImgFiles)
    ...
    if length(fname) > 3
        if strcmp(fname(end-2:end),'jpg') || strcmp(fname(end-2:end),'png') || strcmp(fname(end-2:end), 'dcm')
            if strcmp(fname(end-2:end),'dcm')
               img_dcm = dicomread([ImgDir, '\', fname]);
               [H, img_bone, oldminval]= ContAdjGauss(img_dcm, 255, [0 255]);
               img_bone = uint8(img_bone);
               isDicom = 1;
               img = ContrastAdjustCT_Uint8(img_dcm,40,80); 
            else
...
%     imgAll(i_pos).rota_innerBw = imgAll(i_pos).centered_innerBw;
    ...
    [imgAll(i_pos).mask_roi, imgAll(i_pos).flag] = getinnerbrainwhite2(rota_original_img_2D, imgAll(i_pos).centered_bwSkullBone);
